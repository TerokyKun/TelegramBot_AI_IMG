...
export async function handleMessage(bot, msg) {
    ...
    if (msg.text.match(/^\s*\/start/i) // Добавляем проверку на команду /start
    ) {
        return;
    }
    if (!msg.text ||
    ...
    const match = validatedText.match(/\.prompt:\s*([^.]*)\s*(?:\.negativ:\s*([^.]*)\s*)?(?:\.w:\s*([0-9]+)\s*)?(?:\.h:\s*([0-9]+)\s*)?(?:\.seed:\s*([0-9]+)\s*)?/i);
    if (match) {
        ...
        let widthMsg = match[3] ? parseInt(match[3]) : 512; // Получаем значение ширины
        let heightMsg = match[4] ? parseInt(match[4]) : 512; // Получаем значение высоты
        let seedMsg = match[5] ? parseInt(match[5]) : -1; // Получаем значение seed
        ...
        try {
            ...
            console.log(`Информация о генерируемом изображении:${ingInfo}`);
            // Получаем изображение в формате base64
            const imageBase64 = response.data.images[0];
            // Конвертируем изображение в формат PNG
            const imageBuffer = Buffer.from(imageBase64, "base64");
            const pngBuffer = await sharp(imageBuffer).png().toBuffer();
            const caption = `Генерируемое изображение:

      .prompt: ${useBackIngoImage.prompt}
          
      .negativ: ${useBackIngoImage.negative_prompt}
            
      .seed: ${useBackIngoImage.seed}
                  
      .w: ${useBackIngoImage.width}

      .h: ${useBackIngoImage.height}`;
            ...
            bot.sendMessage(chatId, "Произошла ошибка при генерации изображения. Пожалуйста, попробуйте еще раз.");
        }
    }
}
